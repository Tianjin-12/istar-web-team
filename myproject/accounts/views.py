...
def loginView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                f"/?login_success=true&username={user.username}&timestamp={int(time.time())}"
            )
    else:
...
def registerView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            logger.info(
                f"用户注册成功: username={user.username}, user_id={user.id}, email={user.email}"
            )
            ...
            if authenticatedUser:
                ...
                response = redirect(
                    f"/?register_success=true&username={user.username}&timestamp={int(time.time())}"
                )
                return response
    else:
        form = RegisterForm()
...
def logoutView(request):
    """处理退出登录"""
    logger.info(
        f"用户退出登录: username={request.user.username}, user_id={request.user.id}"
    )
    logout(request)
...
def apiAuthCheck(request):
    ...
    return JsonResponse(
        {"authenticated": False, "username": None, "email": None, "user_id": None}
    )
...
def emailCodeResetPasswordView(request):
    """通过邮箱验证码重置密码（预留）"""
    return JsonResponse({"success": False, "message": "功能开发中"})
...
def send_verification_code(request):
    ...
    if ip_count >= 20:
        return JsonResponse(
            {"success": False, "message": "该IP 1小时内发送次数已达上限（20次）"}
        )

    # 频次限制4：1分钟1邮箱1次（数据库检查，作为第二道防线）
    one_minute_ago = timezone.now() - timedelta(minutes=1)
    if EmailVerificationCode.objects.filter(
        email=email, code_type=code_type, created_at__gte=one_minute_ago
    ).exists():
        return JsonResponse({"success": False, "message": "1分钟内只能发送1次"})
    ...
    try:
        ...
        logger.error(
            f"验证码发送失败: email={email}, code_type={code_type}, error={str(e)}"
        )
...
def register_with_code(request):
    ...
    if len(username) < 2 or len(username) > 8:
        return JsonResponse(
            {"success": False, "message": "用户名长度必须在2-8个字符之间"}
        )
    ...
    try:
        verify_code = EmailVerificationCode.objects.get(
            email=email, code=code, code_type="register", is_used=False
        )
    ...
    if User.objects.filter(email=email).exists():
        return JsonResponse(
            {
                "success": False,
                "message": f'该邮箱已注册。如需重新注册，请访问<a href="{reverse("accounts:deregister_and_reregister")}?email={email}">重新注册</a>',
            }
        )
    ...
    logger.info(
        f"验证码注册成功: username={username}, email={email}, user_id={user.id}"
    )
...
def reset_password_with_code(request):
    ...
    try:
        verify_code = EmailVerificationCode.objects.get(
            email=email, code=code, code_type="reset_password", is_used=False
        )
    ...
    try:
        ...
        logger.info(
            f"密码重置成功: email={email}, username={user.username}, user_id={user.id}"
        )
...
def deregister_and_reregister(request):
    ...
    if request.method != "POST":
        return render(
            request, "accounts/deregister.html", {"email": request.GET.get("email", "")}
        )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "accounts/deregister.html", {"error": "验证码错误"})
