        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                page1.wait_for_selector(
                    "//input[@type='text']", state="visible", timeout=15000
                )
                page1.fill("//input[@type='text']", keyword)
                page1.wait_for_selector(
                    "//button[@aria-label='搜索']", state="visible", timeout=5000
                )
                ...
                for i in range(500):
                    ...
                    page1.evaluate(
                        f"document.documentElement.scrollTop={(i + 1) * 500}"
                    )
                    current_height = page1.evaluate(
...
def load_zhihu_questions(keyword):
    ...
    threshold = datetime.now() - timedelta(days=7)
    questions = list(
        ZhihuQuestion.objects.filter(keyword=keyword, created_at__gte=threshold)
...
def searching_with_db(keyword, use_cache=True):
    ...
    try:
        ...
        for i, row in enumerate(sh.iter_rows(min_row=2, values_only=True), 1):
            if row[0] and row[1]:
                questions.append({"question_id": i, "question_text": row[1]})
