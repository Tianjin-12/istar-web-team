...
def create_3d_scatter(vectors, cluster_map, questions, dimension=3):
    ...
    for cluster_id in sorted(set(cluster_map.values())):
        indices = [
            int(qid) - 1 for qid, cid in cluster_map.items() if cid == cluster_id
        ]
        if len(indices) == 0:
            continue
        ...
        for idx in indices:
            ...
            hover_texts.append(
                f"<b>问题 {qid}</b><br>簇: {cluster_id}<br>{question_text[:80]}..."
            )
        ...
        if dimension == 3:
            ...
            fig.add_trace(
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        size=10 if cluster_id != -1 else 5,
                        color=color,
                        opacity=0.8 if cluster_id != -1 else 0.4,
                    ),
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(
                        size=12 if cluster_id != -1 else 6,
                        color=color,
                        opacity=0.8 if cluster_id != -1 else 0.4,
                    ),
                    name=f"簇 {cluster_id}" if cluster_id >= 0 else "噪声",
    ...
    if dimension == 3:
        fig.update_layout(
            scene=dict(
                xaxis=dict(
                    showgrid=True, gridcolor="rgba(200,200,200,0.3)", title="维度 1"
                ),
                yaxis=dict(
                    showgrid=True, gridcolor="rgba(200,200,200,0.3)", title="维度 2"
                ),
                zaxis=dict(
                    showgrid=True, gridcolor="rgba(200,200,200,0.3)", title="维度 3"
                ),
        ...
        fig.update_layout(
            xaxis=dict(
                title="维度 1", showgrid=True, gridcolor="rgba(200,200,200,0.3)"
            ),
            yaxis=dict(
                title="维度 2", showgrid=True, gridcolor="rgba(200,200,200,0.3)"
            ),
            ...
            [
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                dbc.Label("关键词", className="fw-bold small mb-1"),
                                dcc.Dropdown(
                                    id="keyword-dropdown",
                                    placeholder="选择关键词",
                                    options=[],
                        ...
                        dbc.Col(
                            [
                                dbc.Label("降维方法", className="fw-bold small mb-1"),
                                dcc.Dropdown(
                                    id="dim-method-dropdown",
                                    options=[
                                        {"label": "UMAP (推荐)", "value": "umap"},
                        ...
                        dbc.Col(
                            [
                                dbc.Label("可视化维度", className="fw-bold small mb-1"),
                                dcc.Dropdown(
                        ...
                        dbc.Col(
                            [
                                dbc.Label("", className="fw-bold small mb-1"),
                                dbc.Button(
                                    [html.I(className="bi bi-download me-2"), "导出"],
                                    id="btn-export",
            ...
            [
                dbc.CardBody(
                    [
                        ...
                        html.P(
                            f"有效簇: {stats['valid_clusters']} ({stats['valid_percent']}%)",
                            className="mb-2",
                        ),
                        html.P(
                            f"噪声点: {stats['noise']} ({stats['noise_percent']}%)",
                            className="mb-2",
                        ),
