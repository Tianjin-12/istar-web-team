...
class Mention_percentage(models.Model):
    ...
    high_relevance_ratio = models.FloatField(verbose_name="高相关信源占比", default=0)
    source_stats = models.JSONField(
        verbose_name="信源分类统计快照", default=dict, blank=True
    )
...
class Order(models.Model):
    ...
    task_id = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="任务ID"
    )
    current_stage = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="当前阶段"
    )
...
class LinkCategory(models.Model):
    ...
    relevance_level = models.IntegerField(default=0, verbose_name="相关度等级(0-5)")
    domain_patterns = models.JSONField(
        default=list, verbose_name="自动匹配域名规则", blank=True
    )
    color = models.CharField(
        max_length=20, default="#6c757d", verbose_name="Treemap显示颜色"
    )
    ...
    def __str__(self):
        return f"{self.name}(L{self.relevance_level})"
...
class AILink(models.Model):
    ...
    is_manual = models.BooleanField(default=False, verbose_name="是否手动分类")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
...
class TaskLog(models.Model):
    order = models.ForeignKey(
        Order, on_delete=models.SET_NULL, null=True, db_index=True, verbose_name="订单"
    )
