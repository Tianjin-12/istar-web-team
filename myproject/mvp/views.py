...
def create_order(request):
    ...
    if request.method == "POST":
        ...
        if not keyword or not brand:
            return JsonResponse({"success": False, "message": "请提供关键词和品牌词"})

        existing_order = Order.objects.filter(
            user=request.user, keyword__iexact=keyword, brand__iexact=brand
        ).first()
        ...
        order = Order.objects.create(
            user=request.user, keyword=keyword, brand=brand, status="pending"
        )

        return JsonResponse(
            {"success": True, "message": "订单创建成功", "order_id": order.id}
        )
    ...
    return render(
        request,
        "mvp/create_order.html",
        {"prefilled_brand": prefilled_brand, "prefilled_keyword": prefilled_keyword},
    )
    ...
    if request.method == "GET":
        try:
            ...
            has_order = Order.objects.filter(
                keyword__icontains=keyword, brand__icontains=brand_name
            ).exists()
            if not has_order:
                # 如果没有订单，返回特殊标记
                return JsonResponse(
            ...
            mention_query = Mention_percentage.objects.filter(
                ...
                keyword_name__icontains=keyword,
            )
            # 限制数据量，避免返回过多数据
            ...
            logger.error(f"dashboard_data_api错误: {str(e)}")
            return JsonResponse(
                {"error": "服务器内部错误，请稍后重试", "status": "error"}, status=500
            )
...
def dashboard_view(request):
    ...
    try:
        ...
        dash_app = StatelessApp.objects.create(
            slug="DashboardApp", app_name="DashboardApp"
        )
...
def notification_list(request):
    ...
    return render(
        request, "mvp/notification_list.html", {"notifications": notifications}
    )
...
def cluster_viz_view(request):
    ...
    try:
        ...
        dash_app = StatelessApp.objects.create(
            slug="ClusterVizApp", app_name="ClusterVizApp"
        )
